...
class BaseDataLoaderFuse(DataLoader):
    def __init__(self, dataset, batch_size, shuffle, num_workers, val_split = 0.0):
        ...
        super(BaseDataLoaderFuse, self).__init__(sampler=self.train_sampler, **self.init_kwargs)
...
class DataPrefetcherFuse(object):
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_disp = next(self.loaditer)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_disp = self.next_disp.cuda(device=self.device, non_blocking=True)
