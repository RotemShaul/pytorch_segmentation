...
class CityScapesDatasetRGBD19c(BaseDataSetFuse):
    def __init__(self, mode='fine', **kwargs):
        self.num_classes = 2
        self.mode = mode
        self.palette = palette.CityScpates_palette
        self.id_to_trainId = ID_TO_TRAINID
        super(CityScapesDatasetRGBD19c, self).__init__(**kwargs)
    ...
    def _set_files(self):
        ...
        ##Add disparity paths
        disp_image_path = os.path.join(self.root, 'cityscape_kitti_model_training', self.split) ## TODO

        ###
        image_paths, label_paths, disp_paths = [], [], []
        for city in os.listdir(image_path):
            ...
            disp_paths.extend(sorted(glob(os.path.join(disp_image_path, city, '*.png'))))

        self.files = list(zip(image_paths, label_paths, disp_paths))

    def _load_data(self, index):
        image_path, label_path, disp_path = self.files[index]
        ...
        return image, label, image_id, disp_img



class CityScapesRGBD19c(BaseDataLoaderFuse):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, mode='fine', val=False,
        ...
        self.dataset = CityScapesDatasetRGBD19c(mode=mode, **kwargs)
        super(CityScapesRGBD19c, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
